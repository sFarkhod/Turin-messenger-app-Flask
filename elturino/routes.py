...
from elturino import app, db, bcrypt, mail
...
def new_post():
    form = PostForm()
    if form.validate_on_submit():
        post = Post(title=form.title.data, content=form.content.data, image=save_picture(form.image.data), author=current_user)
        db.session.add(post)
        db.session.commit()
